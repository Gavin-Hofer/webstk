...
export const ImageEditorDialog: React.FC<ImageEditorDialogProps> = ({
  ...
  const onResizePointerMove = useCallback(
    (event: React.PointerEvent<HTMLDivElement>) => {
      ...
      const dx = (event.clientX - startX) * 2;
  ...
  const resizePreview = useMemo(() => {
    ...
    const imageWidth = safeNaturalWidth * scaleX;
    const imageHeight = safeNaturalHeight * scaleY;
    return {
      frameStyle: {
        width: `${resizeConfig.width}px`,
        height: `${resizeConfig.height}px`,
      },
      imageStyle: {
        ...filterStyle,
        width: `${imageWidth}px`,
